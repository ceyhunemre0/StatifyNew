import { getTopTracks, getUserProfile } from '../api.js';
...
async function loadTracks(container) {
  container.innerHTML = `
  ...
  const data = await getTopTracks(currentTimeRange, 20);
  ...
  container.innerHTML = tracks.map((track, i) => `
    <div class="list-item animate-slide-up" style="animation-delay: ${i * 0.03}s">
      ...
      <img src="${track.album?.images?.[1]?.url || track.album?.images?.[0]?.url || ''}" alt="${track.name}" class="list-item-image" />
      <div class="list-item-info">
        <h3 class="list-item-title">${track.name}</h3>
        <p class="list-item-subtitle">${track.artists?.map(a => a.name).join(', ')}</p>
        <p class="list-item-album">${track.album?.name || ''}</p>
      </div>
      <a href="${track.external_urls?.spotify}" target="_blank" rel="noopener" class="track-open-btn" title="Spotify'da aç">
...
export async function renderTopTracks(app) {
  ...
  app.innerHTML = `
    <div class="dashboard">
      ...
      <div class="page-header">
        <h1 class="page-title">🎵 En Çok Dinlediğin Şarkılar</h1>
      ...
      <div class="list-container" id="tracks-list"></div>
